...
func GetPostsByUserID(userID uint32, page uint32, pageSize uint32) ([]*models.Post, error) {
	sql := "SELECT p.id, p.user_id, p.thread_id, p.post_number, p.content, p.created_at, u.username FROM posts p LEFT JOIN users u ON p.user_id = u.id WHERE p.user_id = $1 ORDER BY p.post_number ASC OFFSET $2 LIMIT $3"
